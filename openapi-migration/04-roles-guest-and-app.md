...
**Size:** 2 hand-written method(s) across 2 file(s) → 5 generated operation(s).
...
- `general_api.dart` is split across three groups — only `getAppSettings` is left here. `sync` and `queryMembers` go to group 11, `searchMessages` to group 10. Do not migrate the file as a unit.
