...
- `general_api.dart` is split across four groups — only `enrichUrl` is left here. `sync` and `queryMembers` go to group 11, `searchMessages` to group 10. Do not migrate the file as a unit.
