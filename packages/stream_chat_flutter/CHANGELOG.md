...
- Added `PollTranslationX`, `PollOptionTranslationX` and `PollVoteTranslationX`, with `translatedName`, `translatedText`, `translatedAnswerText`, `hasTranslation` and `translate` helpers mirroring `Message.translatedText` and `Message.translate`; `Message.hasTranslation`, which also counts the message's poll; and `StreamPollCommentsSheet.language` to show its comments translated.
