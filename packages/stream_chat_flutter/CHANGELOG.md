...
- Fixed the channel list still showing a timestamp next to "No messages yet" after a channel is truncated. `ChannelLastMessageDate` now reads the date off the message the preview actually shows instead of `Channel.lastMessageAt`, which cannot be cleared once a truncation removes every message. While the channel holds a historical message window (not up to date), `Channel.lastMessageAt` is still used when it is newer than anything loaded, so a freshly built row cannot show an outdated date.
- Fixed the channel-list preview (subtitle and timestamp) showing messages that don't bump `last_message_at` — system messages on channel types with `skip_last_msg_update_for_system_msgs`, and messages with a `restricted_visibility` excluding the current user — which made a row's content contradict its sort position.
