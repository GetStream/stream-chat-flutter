...
- Removed the unused `shimmer` dependency; the skeleton loading widgets already use `stream_core_flutter`'s `StreamSkeletonLoading` and never depended on the `shimmer` package. This resolves version conflicts for apps that depend on a different `shimmer` major version.
