...
- Extracted the message item's leading avatar rendering into a new internal `StreamMessageLeading` widget, matching the structure used by `StreamMessageHeader` and `StreamMessageFooter`. Behavior is unchanged; `DefaultStreamMessageItem` now composes leading, header, and footer symmetrically.
