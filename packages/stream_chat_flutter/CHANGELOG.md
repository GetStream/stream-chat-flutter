...
- Added `onReactionTap` to `StreamMessageItem` and `StreamMessageListView`, reporting the tapped `Reaction` (or `null` for a clustered or overflow chip that maps to no single reaction).
