...
void main() {
  ...
  testWidgets(
    "rebinding to a not-up-to-date empty channel shows that channel's "
    'empty state',
    (tester) async {
      ...
      when(() => channelStateB.messagesStream)
          .thenAnswer((_) => Stream.value(<Message>[]));
      // B has not caught up yet, so the fallback is live.
      when(() => channelStateB.isUpToDate).thenReturn(false);
