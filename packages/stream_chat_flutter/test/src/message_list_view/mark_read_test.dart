...
void main() {
  ...
  setUp(() {
    ...
    channel = MockChannel();
    // `MockChannel.on()` has a concrete implementation that returns the
    // (default empty) `_eventStream`, so it does not need to be stubbed.
        ...
        MaterialApp(
          home: DefaultAssetBundle(
            ...
            child: StreamChat(
              ...
              // Inject a connectivity stream so `StreamChatCore` doesn't hit
              // the real `connectivity_plus` platform channel (not registered
              // under `flutter test`). Mirror what the real plugin emits — a
              // single seed value — so the first-event skip inside
              // `_subscribeToConnectivityChange` still behaves correctly.
              connectivityStream: Stream.value(const [ConnectivityResult.wifi]),
              child: StreamChannel(
                channel: channel,
  ...
  group('markRead gates', () {
    testWidgets(
      ...
      (tester) async {
        ...
        // `_markMessagesAsRead` goes through a 1 s debounce, so wait it out
        // before verifying.
        await tester.runAsync(() async {
