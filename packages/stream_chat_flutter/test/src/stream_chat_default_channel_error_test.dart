...
void main() {
  ...
  StreamChatNetworkError dioError(DioExceptionType type) =>
        ...
        home: StreamChat(
          ...
          child: Scaffold(
            body: Builder(
              builder: (context) {
                final errorBuilder =
                    DefaultStreamChannelBuilders.errorBuilderOf(context);
  ...
  group('default channel error builder installed by StreamChat', () {
    testWidgets('shows the no-internet copy for connection errors',
        (tester) async {
      await pumpDefaultError(
          tester, dioError(DioExceptionType.connectionError));
    ...
    testWidgets('shows a generic message and never the raw error',
        (tester) async {
      ...
      expect(
          find.textContaining('super secret internal failure'), findsNothing);
    });
  });
          ...
          home: StreamChat(
            ...
            child: Scaffold(
              body: Builder(
                builder: (context) {
                  expectedBackground =
                      StreamChatTheme.of(context).colorTheme.appBg;
                  final loadingBuilder =
                      DefaultStreamChannelBuilders.loadingBuilderOf(context);
