...
class CustomRenderShrinkWrappingViewport extends CustomRenderViewport {
  ...
  void performLayout() {
    ...
    do {
      ...
      if (correction != 0.0) {
        ...
        final didAcceptViewportDimension = offset.applyViewportDimension(effectiveExtent);
        final didAcceptContentDimension = offset.applyContentDimensions(minScrollOffset, maxScrollOffset);
  ...
  double _attemptLayout(
    ...
    assert(crossAxisExtent >= 0.0, 'The crossAxisExtent of $this is negative.');
    assert(
      correctedOffset.isFinite,
      'The correctedOffset of $this is not finite.',
    );
    _minScrollExtent = 0.0;
    _maxScrollExtent = 0.0;
    _hasVisualOverflow = false;
...
abstract class CustomRenderViewport extends RenderViewportBase<CustomSliverPhysicalContainerParentData> {
  ...
  void updateChildLayoutOffset(
    ...
    final childParentData = child.parentData! as CustomSliverPhysicalContainerParentData;
    childParentData
  ...
  void applyPaintTransform(RenderObject child, Matrix4 transform) {
    final offset = paintOffsetOf(child as RenderSliver);
  ...
  int get indexOfFirstChild {
    assert(center != null, 'RenderViewport does not have any children.');
    assert(
      center!.parent == this,
      'center is not a child of this RenderViewport',
    );
    assert(
      firstChild != null,
      'center is the only child of this RenderViewport',
    );
    var count = 0;
    var child = center;
