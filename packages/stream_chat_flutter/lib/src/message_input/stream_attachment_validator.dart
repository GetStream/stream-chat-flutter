...
class StreamAttachmentValidator {
  ...
  StreamChatError? validate(Attachment attachment) {
    ...
    final maxSize =
        config.sizeLimit > 0 ? config.sizeLimit : UploadConfig.defaultSizeLimit;
    if (size > maxSize) {
      return AttachmentTooLargeError(fileSize: size, maxSize: maxSize);
    }
...
class AttachmentTooLargeError extends StreamChatError {
  ...
  const AttachmentTooLargeError({
    ...
    required this.maxSize,
          ...
          'Attachment size $fileSize exceeds the configured limit of '
          '$maxSize bytes.',
        );
