...
import 'package:stream_chat_flutter/src/fullscreen_media/full_screen_media_scope.dart';

/// State mixin for playback controllers (video / audio) that want to
/// behave well inside a `StreamFullScreenMedia` gallery.
///
/// When the widget is hosted inside the gallery, the mixin subscribes to
/// the enclosing [FullScreenMediaScope]'s active page index and toggles
/// playback so:
///
...
///
/// When there is no [FullScreenMediaScope] ancestor, the mixin treats the
/// widget as always active — [autoplay] kicks in on init and the caller
/// controls play / pause manually after that. This lets the same player
/// class be reused outside the gallery without breaking.
mixin StreamVideoPlayerActivityMixin<T extends StatefulWidget> on State<T> {
  ...
  /// `StreamFullScreenMedia`. Ignored when no gallery ancestor is in scope.
  int get pageIndex;
  ...
  ///
  /// Treated as always active when no [FullScreenMediaScope] ancestor is
  /// in scope, so the player still auto-plays / can be controlled
  /// manually outside the gallery.
  ...
  void didChangeDependencies() {
    ...
    final next = FullScreenMediaScope.maybeOf(context)?.activeIndex;
