...
class ChannelQueryDao extends DatabaseAccessor<DriftChatDatabase> with _$ChannelQueryDaoMixin {
  ...
  String _computeHash(ChannelFilter? filter) {
    if (filter == null) {
      return 'allchannels';
    }
  ...
  Future<void> updateChannelQueriesByPredefinedFilter(
    ...
    await batch((it) {
      it
        ...
        ..insert(
          channelQueriesMetadata,
          ChannelQueryMetadataEntity(queryHash: hash, filter: filter, sort: sort),
  ...
  Future<List<String>> getCachedChannelCids(ChannelFilter? filter) {
    final hash = _computeHash(filter);
  ...
  Future<List<ChannelModel>> getChannels({ChannelFilter? filter}) async {
  ...
  Future<(List<ChannelModel>, ChannelFilter?, List<ChannelSort>?)> getChannelsAndSpecByPredefinedFilter(
    String filterName, {
    Map<String, Object?>? filterValues,
    Map<String, Object?>? sortValues,
  }) async {
    ...
    final cidsQuery = (select(channelQueries)..where((c) => c.queryHash.equals(hash))).map((c) => c.channelCid).get();

    final metadataQuery = (select(channelQueriesMetadata)..where((m) => m.queryHash.equals(hash))).getSingleOrNull();
    ...
    final cachedChannels = await (select(channels)..where((c) => c.cid.isIn(cachedCids)))
        .join([
          leftOuterJoin(users, channels.createdById.equalsExp(users.id)),
        ])
        .map((row) {
