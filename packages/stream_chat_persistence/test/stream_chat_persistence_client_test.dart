  ...
  group('client functions', () {
    ...
    group('getChannelState', () {
      test('should work fine', () async {
        ...
        when(() => mockDatabase.channelDao.getChannelByCid(cid)).thenAnswer((_) async => channel);
        when(
          () => mockDatabase.messageDao.getMessagesByCid(cid, messagePagination: any(named: 'messagePagination')),
        ).thenAnswer((_) async => messages);
        when(() => mockDatabase.pinnedMessageDao.getMessagesByCid(cid)).thenAnswer((_) async => messages);
        when(() => mockDatabase.draftMessageDao.getDraftMessageByCid(cid)).thenAnswer((_) async => null);
        ...
        verify(() => mockDatabase.draftMessageDao.getDraftMessageByCid(cid)).called(3);
      });
    });
