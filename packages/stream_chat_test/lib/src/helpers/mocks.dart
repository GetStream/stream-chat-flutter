...
void registerChatFallbackValues() {
  ...
  // A filter carries the model it queries in its type, so mocktail needs one
  // fallback per query surface rather than a single `Filter`.
  registerFallbackValue(const ChannelFilter.raw({}));
  registerFallbackValue(const MemberFilter.raw({}));
  registerFallbackValue(const UserFilter.raw({}));
  registerFallbackValue(const MessageSearchFilter.raw({}));
  registerFallbackValue(const ThreadFilter.raw({}));
  registerFallbackValue(const PollFilter.raw({}));
  registerFallbackValue(const PollVoteFilter.raw({}));
  registerFallbackValue(const DraftFilter.raw({}));
  registerFallbackValue(const ReactionFilter.raw({}));
  registerFallbackValue(const BannedUserFilter.raw({}));
  registerFallbackValue(const MessageReminderFilter.raw({}));

  // Same for a sort, which is always passed as a list.
  registerFallbackValue(const <ChannelSort>[]);
  registerFallbackValue(const <MemberSort>[]);
  registerFallbackValue(const <UserSort>[]);
  registerFallbackValue(const <MessageSearchSort>[]);
  registerFallbackValue(const <ThreadSort>[]);
  registerFallbackValue(const <PollSort>[]);
  registerFallbackValue(const <PollVoteSort>[]);
  registerFallbackValue(const <DraftSort>[]);
  registerFallbackValue(const <ReactionSort>[]);
  registerFallbackValue(const <BannedUserSort>[]);
  registerFallbackValue(const <MessageReminderSort>[]);
}
