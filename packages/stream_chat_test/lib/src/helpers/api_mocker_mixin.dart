...
mixin ApiMockerMixin {
  ...
  /// layer, which always fails asynchronously. Defaults to
  /// [createDefaultNetworkError]'s retriable server failure.
  ///
  ...
  void mockApiFailure<T>(
    ...
    final failure = error ?? createDefaultNetworkError();
  ...
  /// [error] defaults to [createDefaultNetworkError]'s server failure, which
  /// is retriable: the failed first call arms the SDK's retry queue and the
  /// queue's immediate retry attempt succeeds without waiting on backoff
  /// timers.
