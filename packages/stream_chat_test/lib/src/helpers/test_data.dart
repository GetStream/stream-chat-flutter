...
/// Creates a [UserToken] for [userId], decodable by the SDK's token parsing.
///
...
/// accept it.
UserToken createTestToken(String userId) {
...
/// rejected.
///
/// [code] is the backend error code (see [StreamErrorCode]); the default `40`
/// is `tokenExpired`.
...
/// Creates the [StreamApiException] a server-rejected request reports.
///
/// [statusCode] is what decides whether the SDK retries — 5xx and 408 are
/// retriable, every other verdict is not — so a test wanting one or the other
/// names the status the endpoint would answer with. The default 500 is a
/// server that failed to answer, which is retriable.
StreamApiException createDefaultNetworkError({
  StreamErrorCode code = StreamErrorCode.internalError,
  int statusCode = 500,
