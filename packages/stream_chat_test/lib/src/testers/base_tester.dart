...
// Records are still built, only discarded: the client configures the logger
// globally, so silencing it by priority would stop the lazy message closures
// from running and hide a crash inside one.
const _silentLogConfig = StreamLogConfig(
  priority: StreamLogPriority.verbose,
...
abstract base class BaseTester<S> with ApiMockerMixin {
  ...
  ///
  /// The default 40 ([StreamErrorCode.tokenExpired]) fails the attempt with a
  /// [StreamApiException] under the harness's static token; with a token
  ...
  /// retriable [StreamNetworkException].
...
/// asserts the connection; [connect] replaces it entirely. [logConfig] and
  ...
  return test.test(
    ...
    () async {
      await _runZonedGuarded(() async {
        ...
        // The client configures the logger process-wide, so put it back
        // afterwards rather than leaving every later test in this file silent.
        test.addTearDown(StreamLogger.reset);
        ...
        final wsTokenManager = TokenManager.unconfigured()
          ..setTokenProvider(
            testUser.id,
            tokenProvider: tokenProvider ?? TokenProvider.static(testToken),
          );
        ...
        client = StreamChatClient(
          ...
          chatApi: chatApi,
          ws: ws,
          logConfig: logConfig ?? _silentLogConfig,
