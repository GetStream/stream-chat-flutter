...
void main() {
  ...
  group('Resolver edge cases', () {
    group('pollCreatedResolver edge cases', () {
      test('should preserve all event properties', () {
        ...
        expect(resolved.message, equals(message));
        expect(resolved.cid, equals('channel-123'));
        expect(resolved.createdAt, equals(DateTime(2023)));
      });
    ...
    group('pollAnswerCastedResolver edge cases', () {
      test('should preserve all event properties', () {
        final pollVote = PollVote(
          id: 'vote-123',
          answerText: 'My answer',
      ...
      test('should handle pollVote with both answerText and optionId', () {
        // isAnswer is true if answerText is not null, regardless of optionId
        final pollVote = PollVote(
          id: 'vote-123',
          answerText: 'My answer',
          optionId: 'option-1',
      ...
      test('should handle empty answerText as option vote', () {
        final pollVote = PollVote(
          id: 'vote-123',
          answerText: '',
          pollId: 'poll-123',
        ...
        // Empty string is still considered an answer
        expect(resolved, isNull);
      });
    });

    group('pollAnswerRemovedResolver edge cases', () {
      test('should preserve all event properties', () {
        final pollVote = PollVote(
          id: 'vote-123',
          answerText: 'My answer',
    ...
    group('locationSharedResolver edge cases', () {
      test('should preserve all event properties', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
      ...
      test('should handle message with multiple location types', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
          createdByDeviceId: 'device-123',
      ...
      test('should handle location with minimal data', () {
        final location = Location(
          latitude: 0,
          longitude: 0,
          createdByDeviceId: 'device-123',
    ...
    group('locationUpdatedResolver edge cases', () {
      test('should preserve all event properties', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
      ...
      test('should handle location expiring exactly now', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
          createdByDeviceId: 'device-123',
        ...
        // Expiring at exactly now should be considered expired
        expect(resolved, isNull);
      });

      test('should handle location with very far future endAt', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
          createdByDeviceId: 'device-123',
        ...
        final message = Message(
          id: 'message-123',
          text: 'Location valid for a year',
          sharedLocation: location,
          user: User(id: 'user-123'),
        );
    ...
    group('locationExpiredResolver edge cases', () {
      test('should preserve all event properties', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
      ...
      test('should handle location expired long ago', () {
        final location = Location(
          latitude: 40.7128,
          longitude: -74.0060,
          createdByDeviceId: 'device-123',
          endAt: DateTime.now().subtract(const Duration(days: 365)),
        );

        final message = Message(
          id: 'message-123',
          text: 'Location expired a year ago',
          sharedLocation: location,
          user: User(id: 'user-123'),
        );
    ...
    group('multiple resolver chain', () {
      test('should not interfere with each other', () {
        ...
        // Only pollCreatedResolver should match
        expect(pollCreatedResolver(event), isNotNull);
        expect(pollAnswerCastedResolver(event), isNull);
        expect(pollAnswerRemovedResolver(event), isNull);
        expect(locationSharedResolver(event), isNull);
        expect(locationUpdatedResolver(event), isNull);
        expect(locationExpiredResolver(event), isNull);
      });

      test('should handle event with both poll and location', () {
        final poll = Poll(
          id: 'poll-123',
          name: 'Test Poll',
          options: const [
        ...
        // Both resolvers should match
        expect(pollCreatedResolver(event), isNotNull);
        expect(locationSharedResolver(event), isNotNull);
      });
    });

    group('null and empty event handling', () {
      test('should handle event with all null fields', () {
        final event = Event(
          type: EventType.messageNew,
        );

        expect(pollCreatedResolver(event), isNull);
        expect(pollAnswerCastedResolver(event), isNull);
        expect(pollAnswerRemovedResolver(event), isNull);
        expect(locationSharedResolver(event), isNull);
        expect(locationUpdatedResolver(event), isNull);
