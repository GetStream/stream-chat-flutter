...
import '../../mocks.dart' show FakeClientState, MockLogger, MockStreamChatClient;
...
void main() {
  ...
  group('channel', () {
    ...
    test('onChannelCounts updates the member and message counts', () {
      stubChannelState(ChannelState(channel: ChannelModel(cid: 'messaging:test')));

      mutations.onChannelCounts(memberCount: 7, messageCount: 42);

      final updated = capturedChannelState();
      expect(updated.channel?.memberCount, 7);
      expect(updated.channel?.messageCount, 42);
    });

    test('onChannelCounts leaves a null count unchanged', () {
      stubChannelState(
        ChannelState(
      ...
      mutations.onChannelCounts(memberCount: null, messageCount: 43);
