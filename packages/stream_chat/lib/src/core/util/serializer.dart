/// Helpers for moving custom fields in and out of the `extra_data` key of a JSON map.
class Serializer {
  /// A copy of [json] with every key not in [topLevelFields] moved into `extra_data`.
  ///
  ...
  static Map<String, dynamic> moveToExtraDataFromRoot(
    ...
    final knownFields = _knownFieldsCache[topLevelFields] ??= topLevelFields.toSet();
  ...
  // Each list of top level fields as a set, built once, so key lookups stay fast.
