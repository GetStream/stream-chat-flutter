...
class Channel {
  ...
  static Comparator<Channel>? getComparator({
    required String field,
    required int direction,
  }) {
    return switch (field) {
      ChannelSortField.defaultSort => (Channel channelA, Channel channelB) {
          return ComparatorHelper.compare(
            channelA.lastMessageAt ?? channelA.createdAt,
            channelB.lastMessageAt ?? channelB.createdAt,
            direction,
          );
        },
      ChannelSortField.pinnedAt => (Channel channelA, Channel channelB) {
          return ComparatorHelper.compare(
            channelA.membership?.pinnedAt,
            channelB.membership?.pinnedAt,
            direction,
          );
        },
      _ => null,
...
class ChannelClientState {
  ...
  void updateChannelState(ChannelState updatedState) {
    ...
    if (updatedState.membership case final membership?) {
      _channel.client.logger
          .severe('updateChannelState pinned_at: ${membership.pinnedAt}');
    }
