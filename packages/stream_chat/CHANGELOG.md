...
- The user group responses compare by value and gain `copyWith`; `UserGroup` and `UserGroupMember` still compare by value and gain `copyWith`, but no longer extend `Equatable`, so `props` is removed.
