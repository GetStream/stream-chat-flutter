...
- Fixed the `channel.truncated` event handler discarding the channel object sent with the event; `Channel.truncatedAt` and `Channel.lastMessageAt` now reflect the truncation, matching the `channel.updated` handling.
