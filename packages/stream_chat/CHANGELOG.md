...
- `UserGroup`, `UserGroupMember` and the user group responses compare by value and gain `copyWith`; `UserGroup` and `UserGroupMember` no longer extend `Equatable`, so `props` is removed.
