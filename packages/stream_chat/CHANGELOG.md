...
- Fixed `ChannelClientState.watcherCount` staying stale during a session: `user.watching.start` / `user.watching.stop` and `message.new` events now update the watcher count from the server-provided value, instead of only refreshing on a full `channel.watch()` / `queryChannels`.
