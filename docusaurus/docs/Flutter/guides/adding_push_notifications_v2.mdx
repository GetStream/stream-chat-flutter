...
Make sure to check [this section](https://getstream.io/chat/docs/flutter-dart/push_introduction/?language=dart#push-delivery-rules) of the docs to read about the push delivery logic.
...
You can upload your firebase credentials either using dashboard or using the app settings api (available only in backend SDKs).
...
Push notifications behave a bit differently depending if you are using iOS or Android.

Check [here](https://firebase.flutter.dev/docs/messaging/usage#message-types) to understand the difference between **notification** and **data** payloads.

#### iOS

On iOS we send both a **notification** and a **data** payload.

This means you don't need to do anything special to get the notification to show up. However, you might want to handle the data payload to do run some logic when the user taps on the notification.
...
On Android we send only a **data** payload. This is to give the user more flexibility and let them decide what to do with the notification.

For example you can listen and generate a notification from them.

For example, to generate a notification when a **data-only** message is received and the app is in background:
...
Basically, you get the message details using the `getMessage` method and then you can use the [flutter_local_notifications](https://pub.dev/packages/flutter_local_notifications) package to show the actual notification.
...
To do this we need to integrate the package [stream_chat_persistence](https://pub.dev/packages/stream_chat_persistence) in our app that exports a persistence client, learn [here](https://pub.dev/packages/stream_chat_persistence#usage) how to set it up.

Then calling `FirebaseMessaging.onBackgroundMessage(...)` we need to use a TOP-LEVEL or STATIC function to handle background messages; here is an example:
