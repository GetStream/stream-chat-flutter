...
| ☐ | **`LocationCoordinate` equality** | `==` compares with a `1e-7` epsilon while `hashCode` hashes the raw doubles, so the two disagree: `a == b` yet `{a, b}.length == 2`, and `map[b]` misses after `map[a] =` (probed). The epsilon also makes equality **non-transitive** — `a == b`, `b == c`, `a != c` across two 6e-8 steps — so no `hashCode` can ever be consistent with it. It corrupts `distanceTo` as well, which opens with `if (this == other) return 0.meters`: any two points closer than ~1.1cm report exactly 0m, while above the epsilon haversine is accurate to five decimals. Feeds inherits all of it, since `ActivityData` and `FeedData` carry a `LocationCoordinate` field and freezed generates both members over it. No comparable type does this — JTS `Coordinate`, Google Maps Android `LatLng`, `latlong2`, Apple's `CLLocationCoordinate2D`, stream-core-android and stream-core-swift all keep equality exact and answer proximity with a distance or a named method. It is also the only class in core that hand-writes `==`/`hashCode`; sixteen others use `Equatable`, which makes the split unrepresentable. **Fix:** `extends Equatable` with `props => [latitude, longitude]`, delete both members, keep `distanceTo` for proximity. | Nothing, and it is free today — neither chat nor feeds depends on the tolerance. |
...
| ☑ | **`CurrentPlatform.debugCurrentPlatformOverride`** | Landed on core's `main` in #178. | Landed on `main`; **not in released 0.5.0**, and chat already uses it. |
...
its own, and each release chat has to wait for costs more than the change itself.

**What to push first is no longer a question of which row — it is one release.** `$nor` is closed
and `debugCurrentPlatformOverride` has landed, so the old advice here is spent. What matters now is
that chat already *depends* on five core APIs absent from released 0.5.0, so the branch cannot ship
until a release carries all of them. [`DEFERRED.md`](DEFERRED.md) holds the list and is the
authoritative copy. Everything else in this file is genuinely batchable behind it; the retry table
is the strongest of those.
