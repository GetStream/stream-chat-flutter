...
| --- | --- | --- | --- |
| ☐ | Give `_sortByCreatedAt` a tiebreaker, and the same at `client.dart:646` | `(a, b) => a.createdAt.compareTo(b.createdAt)` lets two distinct messages compare equal, so their order is whatever the last code path decided — a merge, an upsert and a fresh query can each pick differently, and an edit can move a message below a same-timestamp neighbour. A tiebreaker makes the order deterministic and costs one compare only on an exact tie. **Decide the key against the other SDKs first**: if Flutter tiebreaks by `id` and iOS/Android/RN do not, the same channel renders in a different order per platform, which is worse than the rare hop. Kept out of the list-extensions migration deliberately — that change is behaviour-neutral (verified over 40k randomized merges) and this one is not. | [01](01-utilities.md) |
