...
| ☐ | Order pinned messages by `pinnedAt`, and stop the merge asserting on them | Two halves of one bug, both pre-existing. The API returns pinned messages `ORDER BY message.pinned_at DESC NULLS LAST`, but every write goes through `_mergeMessagesIntoExisting` → `sortedUpsertAt(compare: _sortByCreatedAt)`, so a pin lands in creation order. On v10 the mis-placement was silent; core's `sortedUpsertAt` asserts its receiver is sorted, so from this phase on the same payload trips that assert in debug — release builds strip it and misplace the message exactly as before. Sorting the payload by `createdAt` on the way in silences the assert but is not the right order, so it was reverted as unrelated to this phase; fix both together by comparing on `pinnedAt` throughout, which needs a null-ordering decision since `Message.pinnedAt` is nullable where `createdAt` is not. Raised on [#2959](https://github.com/GetStream/stream-chat-flutter/pull/2959). | [01](01-utilities.md) |
